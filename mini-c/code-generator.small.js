export default class CodeGenerator {
  ...
  generateFunctionDeclaration(node) {
    ...
    if (node.expression === true) { // Zeigt an, dass der Body eine Expression ist
        ...
        generatedBodyContent = `return ${expressionCode};`; // Kein \n am Ende hier
    ...
    return `${asyncKeyword}function ${functionName}(${paramsString}){${generatedBodyContent}}`;
  }
  ...
  generateIfStatement(node) {
    ...
    let code = `if(${condition}){${consequentCode}}`;
  ...
  generateForStatement(node) {
    ...
    return `for(${initializer};${condition};${increment}){${bodyCode}}`;
  }
  ...
  generateForEachStatement(node) {
    ...
    return `for(let ${variableName} of ${iterable}){${bodyCode}}`;
  }
  ...
  generateLambdaExpression(node) {
    ...
    if (node.hasAmpersandCapture) { // NEU: Prüfen auf Ampersand-Capture
      return `function(${params}){${bodyContent}}`;
    } else {
      return `(${params})=>{${bodyContent}}`;
    }
  }
